...
# Увеличенный размер нужен, чтобы длинные подписи признаков не накладывались друг на друга
...
# Аккуратное размещение длинных подписей по оси X
ax.set_xticklabels(
    ax.get_xticklabels(),
    rotation=35,
    ha='right',
...
# Подписи по оси Y оставляем горизонтально
ax.set_yticklabels(
    ax.get_yticklabels(),
    rotation=0
)
...
# Дополнительные отступы снизу и справа, чтобы подписи и цветовая шкала не съезжали
plt.subplots_adjust(bottom=0.28, right=0.88)
