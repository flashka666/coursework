...
Соответствие рисунков и подписей в документе (Глава 2):
  ...
  fig06 -> Рисунок 6.  Сравнение диапазонов значений всех каналов (нормировка [0, 1])
...
# ══════════════════════════════════════════════════════════════════════════════
# РИСУНОК 6. Диапазоны значений (нормированные, один график)
# Подпись: Рисунок 6. Сравнение диапазонов значений всех каналов (нормировка [0,1])
# ══════════════════════════════════════════════════════════════════════════════
df_norm  = (df[channels] - df[channels].min()) / (df[channels].max() - df[channels].min())
bp_data  = [df_norm[ch].dropna().values for ch in channels]
...
ax.set_title('Сравнение диапазонов значений всех каналов\n(нормировка [0, 1])',
             fontweight='bold')
ax.set_ylabel('Нормированное значение'); ax.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig(f'{OUT_DIR}/fig06_ranges.png', bbox_inches='tight', dpi=150)
plt.close()
print('fig06 saved  →  Рисунок 6. Диапазоны значений (нормированные)')
...
Таблица соответствия файлов и подписей (Глава 2):
  ...
  figures_ch2/fig06_ranges.png             → Рисунок 6.  Диапазоны значений (нормировка [0,1])
